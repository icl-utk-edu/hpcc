...
void
HPCC_Power2NodesMPIRandomAccessCheck_LCG(u64Int logTableSize,
                                u64Int TableSize,
  ...
  while(LocalAllDone == HPCC_FALSE){
    if (SendCnt > 0) {
      ...
      while(NextSlot != (BUCKET_SIZE+FIRST_SLOT) && SendCnt>0 ) {
        ...
        WhichPe = (Ran >> (64 - logNumProcs)) & (NumProcs - 1);
    ...
    for (i = 0; i < NumProcs; i ++) {
      if(PeCheckDone[i] == HPCC_FALSE) {
        ...
        for (j = FIRST_SLOT; j < n; ++j) {
          ...
          HPCC_Table[(RanTmp >> (64 - logTableSize)) & (LocalTableSize-1)] ^= RanTmp;
        }
...
void
HPCC_AnyNodesMPIRandomAccessCheck_LCG(u64Int logTableSize,
                             u64Int TableSize,
                             s64Int LocalTableSize,
  ...
  while(LocalAllDone == HPCC_FALSE){
    ...
    for (i = 0; i < NumProcs; i ++) {
      if(PeCheckDone[i] == HPCC_FALSE) {
        ...
        for (j = FIRST_SLOT; j < n; ++j) {
          RanTmp = GlobalBuckets[PeBucketBase+j];
          GlobalOffset = RanTmp >> (64 - logTableSize);
